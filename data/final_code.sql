SELECT 	s2.name,
	...
	ROUND((s2.p_longevity_years + a_longevity_years) / 2, 2 ) * 1200 AS total_subscription_cost,
	ROUND((s2.p_longevity_years + a_longevity_years) / 2, 2 ) * 30000 AS expected_revenue,
	(ROUND((s2.p_longevity_years + a_longevity_years) / 2, 2 ) * 30000) - (s2.play_purchase_price + s2.apple_purchase_price + 
	ROUND((s2.p_longevity_years + a_longevity_years) / 2, 2 ) * 1200) AS total_roi	
FROM  (
	SELECT 	sub.name,
